...
#define VOLTAGE_FULL_RANGE  1500	/* VA voltage default  900 */ 
				  ...
				  long info)
{
	...
	case IIO_CHAN_INFO_RAW:
		*val = mt6577_auxadc_read(indio_dev, chan);
		printk(KERN_ERR "Thermal IIO_CHAN_INFO_RAW mt6577_auxadc_read_raw %d chan->channel = %d \n",*val,chan->channel);
		if (*val < 0) {
	...
	case IIO_CHAN_INFO_PROCESSED:
		*val = mt6577_auxadc_read(indio_dev, chan);
		printk(KERN_ERR "Thermal 1 IIO_CHAN_INFO_PROCESSED mt6577_auxadc_read_raw %d chan->channel = %d\n",*val,chan->channel);
		if (*val < 0) {
			dev_err(indio_dev->dev.parent,
				"failed to sample data on channel[%d]\n",
				chan->channel);
		...
		*val = *val * VOLTAGE_FULL_RANGE / AUXADC_PRECISE;
		printk(KERN_ERR "Thermal 2 IIO_CHAN_INFO_PROCESSED mt6577_auxadc_read_raw %d chan->channel = %d  adc_dev->dev_comp->sample_data_cali = %d \n",*val,chan->channel,adc_dev->dev_comp->sample_data_cali);
		return IIO_VAL_INT;
