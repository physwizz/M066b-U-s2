...
//#include "mtk_spi.h" //Deletable
//#include "mtk_spi_hal.h" //Deletable
...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    if ( fp_dev->avdd_ldo ) {
		LOG_MSG_DEBUG(INFO_LOG, "[%s] avdd_ido enter.\n", __func__);
        err = regulator_set_voltage(fp_dev->avdd_ldo, AVDD_MIN, AVDD_MAX);	/*set 2.8v*/
		LOG_MSG_DEBUG(INFO_LOG, "%s: set avdd-voltage ret:%d \n", __func__, err);
        err = regulator_enable(fp_dev->avdd_ldo);	/*enable regulator*/
		LOG_MSG_DEBUG(INFO_LOG, "%s: avdd-enable ret:%d \n", __func__, err);
    }
    if ( fp_dev->vddio_ldo ) {
		LOG_MSG_DEBUG(INFO_LOG, "[%s] vddio_ido enter.\n", __func__);
        err = regulator_set_voltage(fp_dev->vddio_ldo, VDDIO_MIN, VDDIO_MAX);	/*set 1.8v*/
		LOG_MSG_DEBUG(INFO_LOG, "%s: set vddio-voltage ret:%d \n", __func__, err);
        err = regulator_enable(fp_dev->vddio_ldo);	/*enable regulator*/
		LOG_MSG_DEBUG(INFO_LOG, "%s: vddio-enable ret:%d \n", __func__, err);
    }
...
{   int err = 0;
    ...
    if ( fp_dev->avdd_ldo && (regulator_is_enabled(fp_dev->avdd_ldo) > 0)) {
	  ...
	  LOG_MSG_DEBUG(INFO_LOG, "%s: avdd_ido turn off ret:%d \n", __func__, err);
    }
    if ( fp_dev->vddio_ldo && (regulator_is_enabled(fp_dev->vddio_ldo) > 0)) {
      err = regulator_disable(fp_dev->vddio_ldo);   /*disable regulator*/
	  LOG_MSG_DEBUG(INFO_LOG, "%s: vddio_ido turn off ret:%d \n", __func__, err);
    }
#endif /* BSP_SIL_POWER_SUPPLY_REGULATOR */
...
{
#if (!IS_ENABLED(CONFIG_SILEAD_FP_PLATFORM))
#if (!IS_ENABLED(CONFIG_MT_SPI_FPGA_ENABLE))
#if IS_ENABLED(CONFIG_MTK_CLKMGR)
    if (enable) {
    ...
    LOG_MSG_DEBUG(DBG_LOG, "[%s] fp_dev->spionoff_count:%d,done (%d).\n",__func__, fp_dev->spionoff_count, ret);
#endif /* CONFIG_MTK_CLKMGR */
