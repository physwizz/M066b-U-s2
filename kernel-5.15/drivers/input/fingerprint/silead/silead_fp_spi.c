...
#include "silead_fp.h"
#if IS_ENABLED(CONFIG_SILEAD_FP_PLATFORM) && defined(BSP_SIL_PLAT_MTK)
...
#endif /* CONFIG_SPI_MT65XX */
#endif /* CONFIG_SILEAD_FP_PLATFORM BSP_SIL_PLAT_MTK */
...
static int silfp_ree_parse_spi_dts(struct silfp_ree_data* ree_data)
{
    ...
    do {
        ...
        if (!node) {
            LOG_MSG_DEBUG(INFO_LOG, "mediatek,spidev-pins device is null\n");
            ret = -1;
            break;
        }
