...
static nav_keymap_t keymap[] = {
    ...
    { NAV_KEY_DCLICK,   KEY_RESERVED/*KEY_HOMEPAGE*/,   },
...
struct silfp_data *silfp_fb_dev = NULL;
...
#else
static int silead_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    case MTK_DISP_BLANK_UNBLANK:
        ...
        silfp_fb_dev->scr_off = 0;
        silfp_netlink_send(silfp_fb_dev, SIFP_NETLINK_SCR_ON);
        break;

    case MTK_DISP_BLANK_POWERDOWN:
        LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD OFF\n", __func__);
        silfp_fb_dev->scr_off = 1;
        silfp_netlink_send(silfp_fb_dev, SIFP_NETLINK_SCR_OFF);
        break;
...
static int silfp_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%s%s", "[Fingerprint]:silead_fp  [IC]:", vendor_name);
    return 0;
}
...
static int silfp_init(struct silfp_data *fp_dev)
{
    ...
    if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)){
        ...
        fp_dev->notif.notifier_call = silead_fb_notifier_callback;
        mtk_disp_notifier_register("silead_fb_notifier", &fp_dev->notif);
    }
#endif /* CONFIG_HAS_EARLYSUSPEND */
