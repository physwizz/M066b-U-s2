...
static struct gf_device *gf_fb_dev = NULL;
/*************************************************************/
...
static struct notifier_block gf_fb_notifier = {
		...
		.notifier_call = gf_fb_notifier_callback
 };
...
{
	seq_printf(file,"[Fingerprint]:goodix_fp  [IC]:GF3986\n");
	return 0;
}

static int proc_open(struct inode *inode,struct file *file)
{
	printk("goodix proc_open\n");
...
int gf_parse_dts(struct gf_device *gf_dev)
{
	...
	gf_debug(DEBUG_LOG, "%s, goodix_rst=%d\n", __func__,gf_dev->reset_gpio);
	...
	gf_debug(DEBUG_LOG, "%s, success to reset\n", __func__);
	...
	/* TODO:  set power  according to actual situation  */
		...
		if(NULL == gf_dev->vdd) {
			...
			if(IS_ERR(gf_dev->vdd)) {
				pr_err("gf_hw_power_enable---- unable to get vdd ----\n");
				return;
			} else {
				pr_info("gf_hw_power_enable ---- get vdd ok  ----\n");
			}
		}
		if(NULL != gf_dev->vdd) {
			if(regulator_is_enabled(gf_dev->vdd)) {
				pr_info("gf_hw_power_enable---- vdd is already enabled  ----%d\n",
							regulator_is_enabled(gf_dev->vdd));
				return;
			}
			//rc = regulator_set_load(gf_dev->vdd,600000);
			if(rc<0) {
				pr_err("gf_hw_power_enable---- unable to regulator_set_load ----\n");
			}
			if(regulator_count_voltages(gf_dev->vdd)>0) {
				rc = regulator_set_voltage(gf_dev->vdd,3300000,3300000);
				if(rc) {
					pr_err("gf_hw_power_enable---- unable to regulator_set_voltage ----\n");
					return;
				}else {
					pr_err("gf_hw_power_enable-- set_voltage 3.3 --\n");
				}
			}else{
				pr_err("gf_hw_power_enable-regulator_count_voltage: %d\n",regulator_count_voltages(gf_dev->vdd));
			}
			rc = regulator_enable(gf_dev->vdd);
		}
#endif
        if (rc) {
            pr_err("gf_hw_power_enable---- power on failed rc = %d ----\n", rc);
        } else {
			regulator_flag++;
            pr_info("gf_hw_power_enable-power on ok-regulator_flag = %d\n", regulator_flag);
        }
#ifdef CONFIG_OF
		/*if(!IS_ERR(gf_dev->pins_reset_low)) {
		...
		if (regulator_flag) {
			if(NULL != gf_dev->vdd) {
				...
				if(rc<0) {
					pr_err("gf_hw_power_enable---- unable to regulator_set_load 0 ----\n");
				}
				if(regulator_is_enabled(gf_dev->vdd)) {
					regulator_disable(gf_dev->vdd);
					pr_info("gf_hw_power_enable---- regulator_disable  ----\n");
				}
			}
		}
#endif
        if (rc) {
            pr_err("gf_hw_power_enable---- power off failed rc = %d ----\n", rc);
        } else {
			regulator_flag--;
            pr_info("gf_hw_power_enable-power off ok-regulator_flag=%d\n", regulator_flag);
        }
	}
}
...
static int gf_fb_notifier_callback(struct notifier_block *notif,
			unsigned long event, void *data)
{
	struct gf_device *dev = NULL;
	...
	dev = container_of(self, struct gf_device, notifier);
	...
	case FB_BLANK_UNBLANK:
		...
		gf_netlink_send(dev, GF_NETLINK_SCREEN_ON);
		break;

	case FB_BLANK_POWERDOWN:
		gf_debug(INFO_LOG, "[%s] : lcd off notify\n", __func__);
		gf_netlink_send(dev, GF_NETLINK_SCREEN_OFF);
		break;
...
static int gf_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if(gf_fb_dev == NULL)
	{
		gf_debug(INFO_LOG, "[%s] : gf_fb_dev is NULL\n", __func__);
		return 0;
	}
	...
	case MTK_DISP_BLANK_UNBLANK:
		gf_debug(INFO_LOG, "[%s] : lcd on notify\n", __func__);
		gf_netlink_send(gf_fb_dev, GF_NETLINK_SCREEN_ON);
		break;

	case MTK_DISP_BLANK_POWERDOWN:
		gf_debug(INFO_LOG, "[%s] : lcd off notify\n", __func__);
		gf_netlink_send(gf_fb_dev, GF_NETLINK_SCREEN_OFF);
		break;
...
#else
		//register fb_callback
		ret = mtk_disp_notifier_register("gf_fp_notifier", &gf_fb_notifier);
		if(ret)
			gf_debug(INFO_LOG, "%s: fb_mtk_callback regist fail ======\n", __func__);
		else
			gf_debug(INFO_LOG, "%s: fb_mtk_callback regist success ======\n", __func__);
...
#else
		//fb_unregister_client(&gf_fb_notifier);
		mtk_disp_notifier_unregister(&gf_fb_notifier);
		gf_debug(INFO_LOG, "%s: fb_callback unregist success\n", __func__);
#endif
...
#else
		//register fb_callback
		ret = mtk_disp_notifier_register("gf_fp_driver", &gf_fb_notifier);
		if(ret)
			gf_debug(INFO_LOG, "%s: fb_mtk_callback regist fail ======\n", __func__);
		else
			gf_debug(INFO_LOG, "%s: fb_mtk_callback regist success ======\n", __func__);
...
static int gf_release(struct inode *inode, struct file *filp)
{
	...
	if(!gf_fp_adm){
		...
		gf_adm_release = 0;
    		printk("goodixfp fingerprint_adm_device remove!");
	}
    	#endif
	FUNC_ENTRY();
...
#else
	mtk_disp_notifier_unregister(&gf_fb_notifier);
	gf_debug(INFO_LOG, "%s: fb_callback unregist success by cc\n", __func__);
#endif
