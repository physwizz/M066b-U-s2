...
static void odm_get_hwver_gpio(struct device_node *node, int *board_id1, int *board_id2)
{
	int gpio_board_id1,gpio_board_id2;

	gpio_board_id1 = of_get_named_gpio(node, "odm,gpio-board-id1", 0);
	gpio_board_id2 = of_get_named_gpio(node, "odm,gpio-board-id2", 0);
	if (gpio_board_id2 < 0 || gpio_board_id2 < 0) {
		pr_notice("failed to get id1:%d,id2:%d\n", gpio_board_id1, gpio_board_id2);
		return;
	}
	gpio_direction_input(gpio_board_id1);
	gpio_direction_input(gpio_board_id2);

	*board_id1 = gpio_get_value(gpio_board_id1);
	*board_id2 = gpio_get_value(gpio_board_id2);
}
				...
				struct device_node *node)
{
	...
	odm_get_hwver_gpio(node, &board_id1, &board_id2);
	if((board_id1 == 1) && (board_id2 == 0)){
		pr_notice("board_id1:%d,board_id2:%d\n", board_id1, board_id2);
		ret = of_property_read_u32_array(node, "mediatek,hw-init-map-dvt",
