...
{
    //Trigger_CMD
    {STK_ADDR_TRIGGER_REG,          STK_TRIGGER_REG_INIT_ALL},
    ...
    {STK_ADDR_CORR_ENGA_0,        0x0},//PH0&1    ref PH0
    {STK_ADDR_CORR_ENGA_1,        0x0},
    {STK_ADDR_CORR_ENGB_0,        0x5},//PH2&4    ref PH5
    {STK_ADDR_CORR_ENGB_1,        0x0},
    {STK_ADDR_CORR_ENGC_0,        0x3},//PH5&6    ref PH3
    ...
    {STK_ADDR_DELTADES_C_CTRL, STK_ADDR_DELTADES_C_CTRL_VALUE}, //enable delta descend C
#endif
};
...
void temperature_compensation(struct stk_data *stk, uint32_t int_flag, uint16_t prox_flag)
{
    ...
    else if (int_flag & STK_IRQ_SOURCE_ENABLE_REG_DELTA_DES_IRQ_EN_MASK)
    {
        ...
        if (delta_des & STK_DETECT_STATUS_4_DES_STAT_B_MASK)
        {
            ...
            {
                reg  = STK_ADDR_FILT_CFG_PH0 + (DELTA_B_MEASURE_PHASE * 0x40);
                STK_REG_READ(stk, reg, (uint8_t*)&val);
                val |= BASE_REINIT_DELTA_DES;
                STK_REG_WRITE(stk, reg, (uint8_t*)&val);

                stk->reinit[DELTA_B_MEASURE_PHASE] = 1;
                stk->last_prox_b_state = 0;
            }
            else //reset descent
            {
        ...
        if (delta_des & STK_DETECT_STATUS_4_DES_STAT_C_MASK)
        {
            ...
            {
                reg  = STK_ADDR_FILT_CFG_PH0 + (DELTA_C_MEASURE_PHASE * 0x40);
                STK_REG_READ(stk, reg, (uint8_t *)&val);
                val |= BASE_REINIT_DELTA_DES;
                STK_REG_WRITE(stk, reg, (uint8_t *)&val);
                
                reg  = STK_ADDR_FILT_CFG_PH0 + (6 * 0x40);
                STK_REG_READ(stk, reg, (uint8_t *)&val);
                val |= BASE_REINIT_DELTA_DES;
                STK_REG_WRITE(stk, reg, (uint8_t *)&val);

                stk->reinit[DELTA_C_MEASURE_PHASE] = stk->reinit[6] = 1;
                stk->last_prox_c_state = 0;
            }
            else // reset descent
            {
...
void stk501xx_set_enable(struct stk_data* stk, char enable)
{
    ...
    if (enable)
    {
        if(pause_mode == 1) {
            ...
            if ((val & (STK_TRIGGER_REG_INIT_ALL)) != STK_TRIGGER_REG_INIT_ALL)
            {
...
#if 0
/*
 * @brief: disable conversation done interrupt check.
 *
 * @param[in/out] stk: struct stk_data *
 */

static void disable_conv_check(struct stk_data *stk)
{
    uint8_t  need_to_check = 0;
    uint16_t reg = 0;
    uint32_t val = 0;
    ...
    {
        need_to_check = 1;
        STK_LOG("turn off descent int check\n");
        stk->descent_fired = 0;
    }
#endif
...
#ifdef TEMP_COMPENSATION
        stk->reinit[i] = 0;
        stk->descent_cnt_a = stk->descent_cnt_b = stk->descent_cnt_c = \
        stk->descent_fired = 0;
#endif
    }
...
void stk_work_queue(void *stkdata)
{
    ...
    {
        STK_ERR("sensing wdt trigger, (en: %d), stk->skip_wdt(en: %d)\n", stk->enabled, stk->skip_wdt);
        if(stk->enabled && (stk->skip_wdt == false))
        {
...
void stk_report_sar_data(struct stk_data* stk)
{
    ...
    for (i = 0; i < ch_num; i ++)// report channel[1] channel[2] refer each IC phase
    {
        if(stk_wrapper->channels[i].enabled)
        {
            is_change = stk->state_change[mapping_phase[i]];
            STK_ERR("stk_report_sar_data:: change ph[%d] =%d,(%d)", mapping_phase[i], stk->state_change[mapping_phase[i]], is_change);
            ...
            if(stk->force_detected  == 1)
            {
                STK_ERR("force detected near\n");
                is_change = 1;
                nf_flag = SAR_STATE_NEAR;
            }
            else if(stk->force_detected  >= 2)
            {
                STK_ERR("force detected far\n");
                is_change = 1;
                nf_flag = SAR_STATE_FAR;
            }
            ...
            if (is_change != 0 && global_stk->power_en == 0)
            {
                /*with ss sensor hal*/
//                wake_lock_timeout(&stk501xx_wake_lock, HZ * 1);
                STK_ERR("stk report ch[%d], state = %d",i,nf_flag);
                input_report_rel(stk_wrapper->channels[i].input_dev, REL_MISC, nf_flag);
                if(stk->anfr_status)
...
{
    int32_t ch0_diff = 0;
    int32_t ch0_cadc = 0, ch0_cadc_ref = 0;

    STK_ERR("class_stk_value_show_ch0");

    ch0_diff = global_stk->last_data[mapping_phase[0]];
    ch0_cadc = global_stk->last_cadc[mapping_phase[0]];
    ch0_cadc_ref = global_stk->last_cadc[mapping_phase[5]];

    STK_ERR("CH0_ref_cap=%d;CH0_background_cap=%d;CH0_diff=%d;\n", ch0_cadc_ref, ch0_cadc, ch0_diff);
    return scnprintf(buf, PAGE_SIZE, "CH0_ref_cap=%d;CH0_background_cap=%d;CH0_diff=%d\n", ch0_cadc_ref, ch0_cadc, ch0_diff);
}
                                        ...
                                        struct class_attribute *attr, char *buf)
{
    ...
    int32_t ch1_diff = 0;
    int32_t ch1_cadc = 0, ch1_cadc_ref = 0;

    STK_ERR("class_stk_value_show_ch1");

    ch1_diff = global_stk->last_data[mapping_phase[1]];
    ch1_cadc = global_stk->last_cadc[mapping_phase[1]];
    ch1_cadc_ref = global_stk->last_cadc[mapping_phase[5]];

    STK_ERR("CH1_ref_cap=%d;CH1_background_cap=%d;CH1_diff=%d;\n", ch1_cadc_ref, ch1_cadc, ch1_diff);
    return scnprintf(buf, PAGE_SIZE, "CH1_ref_cap=%d;CH1_background_cap=%d;CH1_diff=%d\n", ch1_cadc_ref, ch1_cadc, ch1_diff);
}
...
{
    int32_t ch2_diff = 0;
    int32_t ch2_cadc = 0, ch2_cadc_ref = 0;

    STK_ERR("class_stk_value_show_ch2");

    ch2_diff = global_stk->last_data[mapping_phase[2]];
    ch2_cadc = global_stk->last_cadc[mapping_phase[2]];
    ch2_cadc_ref = global_stk->last_cadc[mapping_phase[6]];

    STK_ERR("CH2_ref_cap=%d;CH2_background_cap=%d;CH2_diff=%d;\n", ch2_cadc_ref, ch2_cadc, ch2_diff);
    return scnprintf(buf, PAGE_SIZE, "CH2_ref_cap=%d;CH2_background_cap=%d;CH2_diff=%d\n", ch2_cadc_ref, ch2_cadc, ch2_diff);
}
...
{
    int32_t ch3_diff = 0;
    int32_t ch3_cadc = 0, ch3_cadc_ref = 0;

    STK_ERR("class_stk_value_show_ch3");

    ch3_diff = global_stk->last_data[mapping_phase[3]];
    ch3_cadc = global_stk->last_cadc[mapping_phase[3]];
    ch3_cadc_ref = global_stk->last_cadc[mapping_phase[4]];

    STK_ERR("CH3_ref_cap=%d;CH3_background_cap=%d;CH3_diff=%d;\n", ch3_cadc_ref, ch3_cadc, ch3_diff);
    return scnprintf(buf, PAGE_SIZE, "CH3_ref_cap=%d;CH3_background_cap=%d;CH3_diff=%d\n", ch3_cadc_ref,ch3_cadc, ch3_diff);
}

static ssize_t class_stk_show_ch0_state(struct class *class,
                                        struct class_attribute *attr, char *buf)
{
    int8_t near_states = 0;
    stk501xx_wrapper *stk_wrapper = container_of(global_stk, stk501xx_wrapper, stk);

    STK_ERR("class_stk_show_ch0_state");

    if(stk_wrapper->channels[0].enabled)
    {
    if(global_stk->last_nearby[mapping_phase[0]] == STK_SAR_NEAR_BY)
            near_states = SAR_STATE_NEAR;
    else if(global_stk->last_nearby[mapping_phase[0]] == STK_SAR_FAR_AWAY)
            near_states = SAR_STATE_FAR;
    }
    else
    {
        near_states = global_stk->ch_status[0];
    }

    if(global_stk->force_detected  == 1)
    {
        STK_ERR("force detected near\n");
        near_states = SAR_STATE_NEAR;
    }
    else if(global_stk->force_detected  >= 2)
    {
        STK_ERR("force detected far\n");
        near_states = SAR_STATE_FAR;
    }
    ...
    STK_ERR("CH0 state=%d\n", near_states);
    return scnprintf(buf, PAGE_SIZE, "CH0 state=%d\n", near_states);
}

static ssize_t class_stk_show_ch1_state(struct class *class,
                                        struct class_attribute *attr, char *buf)
{
    int8_t near_states = 0;
    ...
    STK_ERR("class_stk_show_ch1_state");

    if(stk_wrapper->channels[1].enabled)
    {
    if(global_stk->last_nearby[mapping_phase[1]] == STK_SAR_NEAR_BY)
            near_states = SAR_STATE_NEAR;
    else if(global_stk->last_nearby[mapping_phase[1]] == STK_SAR_FAR_AWAY)
            near_states = SAR_STATE_FAR;
    }
    else
    {
        near_states = global_stk->ch_status[1];
    }
    ...
    STK_ERR("CH1 state=%d\n", near_states);
    return scnprintf(buf, PAGE_SIZE, "CH1 state=%d\n", near_states);
}

static ssize_t class_stk_show_ch2_state(struct class *class,
                                        struct class_attribute *attr, char *buf)
{
    int8_t near_states = 0;

    stk501xx_wrapper *stk_wrapper = container_of(global_stk, stk501xx_wrapper, stk);
    STK_ERR("class_stk_show_ch2_state");

    if(stk_wrapper->channels[2].enabled)
    {
    if(global_stk->last_nearby[mapping_phase[2]] == STK_SAR_NEAR_BY)
            near_states = SAR_STATE_NEAR;
    else if(global_stk->last_nearby[mapping_phase[2]] == STK_SAR_FAR_AWAY)
            near_states = SAR_STATE_FAR;
    }
    else
    {
        near_states = global_stk->ch_status[2];
    }
    ...
    STK_ERR("CH2 state=%d\n", near_states);
    return scnprintf(buf, PAGE_SIZE, "CH2 state=%d\n", near_states);
}

static ssize_t class_stk_show_ch3_state(struct class *class,
                                        struct class_attribute *attr, char *buf)
{
    int8_t near_states = 0;

    stk501xx_wrapper *stk_wrapper = container_of(global_stk, stk501xx_wrapper, stk);
    STK_ERR("class_stk_show_ch3_state");

    if(stk_wrapper->channels[3].enabled)
    {
    if(global_stk->last_nearby[mapping_phase[3]] == STK_SAR_NEAR_BY)
            near_states = SAR_STATE_NEAR;
    else if(global_stk->last_nearby[mapping_phase[3]] == STK_SAR_FAR_AWAY)
            near_states = SAR_STATE_FAR;
    }
    else
    {
        near_states = global_stk->ch_status[3];
    }
    ...
    STK_ERR("CH3 state=%d\n", near_states);
    return scnprintf(buf, PAGE_SIZE, "CH3 state=%d\n", near_states);
}
...
static ssize_t stk_channel_en_store(struct class *class,
        struct class_attribute *attr, const char *buf, size_t count)
{
    uint8_t ch_idx, cmd_len = 0, i = 0, en = 0;
    u32 val;

    stk501xx_wrapper *stk_wrapper = container_of(global_stk, stk501xx_wrapper, stk);
    
    cmd_len = sscanf(buf, "%x,%d", &ch_idx, &en);

    if(cmd_len !=2)
    {
        cmd_len = sscanf(buf, "%x", &ch_idx);
    }

    if ( (cmd_len !=2) && (cmd_len !=1) )
    {
        STK_ERR(" Enable/Disable one chaneel usage: \
                  please input two Hex numbers: ch_id,en \n");

        STK_ERR(" Enable/Disable multiple chaneel usage: \
                  please input one Hex numbers: ch_id \
                  (ch_id: channel number(0:ch0,1:ch1,2:ch2,3:ch3,), other channel wiil be disable\n");
        return -EINVAL;
    }

    if ((ch_idx > 0xF) || (ch_idx < 0))
    {
        STK_ERR("channel idx must less than 0x0F and >= 0");
        return count;
    }
    ...
    if(cmd_len == 1)
    {
        for(i = 0; i < ch_num; i++)
        {
            if(ch_idx & (1<<i))
            {
                ...
                stk_wrapper->channels[i].enabled = 1;
                ...
                global_stk->state_change[mapping_phase[i]] = 1;       //force update
                global_stk->ch_status[i] = SAR_STATE_NONE;
#ifdef STK_POLLING_MODE
...
#endif
                STK_ERR("enable sar, reset all phase\n");
                stk501xx_phase_reset(global_stk, STK_TRIGGER_REG_INIT_ALL);
            }
            else
            {
                stk_wrapper->channels[i].enabled = 0;
                stk_chk_sar_en_irq(stk_wrapper, i);
                global_stk->ch_status[i] = ((global_stk->last_nearby[mapping_phase[i]]) == STK_SAR_NEAR_BY )? SAR_STATE_NEAR: SAR_STATE_FAR;
            }
        }
    }
    ...
    if(cmd_len == 2)
    {
        ...
        if(stk_wrapper->channels[ch_idx].enabled)
            global_stk->ch_status[ch_idx] = SAR_STATE_NONE;
        else //disable
        {
            global_stk->ch_status[ch_idx] = ((global_stk->last_nearby[mapping_phase[ch_idx]]) == STK_SAR_NEAR_BY) ? SAR_STATE_NEAR: SAR_STATE_FAR;
        }
...
static ssize_t anfr_show(struct class *class,
        struct class_attribute *attr, char *buf)
{
    int ret = 0;
    int value = 0;
    value = global_stk->anfr_status;

    STK_ERR("read anfr_status = %d\n", value);
    ret = sprintf(buf, "%d\n", value);
    return ret;
}

static ssize_t anfr_store(struct class *class,
    struct class_attribute *attr, const char *buf, size_t count)
{
    int value = 0;
    ...
    {
        STK_ERR("Invalid command format");
        return -EINVAL;
    }
    STK_ERR("value = %d", value);
    global_stk->anfr_status = value;

    return count;
}
        
static ssize_t stk_debug_show(struct class *class,
                                    struct class_attribute *attr, char *buf)
{
    //int result = 0;
    STK_ERR("stk_debug_show\n");
    
    return sprintf(buf, "Current force report: %d(0: noraml report, 1:Near, other:Far)\n", 
                        global_stk->force_detected);
}

static ssize_t stk_debug_store(struct class *class,
        struct class_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    ret = kstrtouint(buf, 10, &global_stk->force_detected);
    if (0 != ret) {
        STK_ERR("kstrtoint failed\n");
    }
    stk_report_sar_data(global_stk);
...
static struct class_attribute class_attr_enable =
    __ATTR(enable, 0664, stk_channel_en_show, stk_channel_en_store);
...
static struct class_attribute class_attr_ch0_state=
    __ATTR(ch0_state, 0444, class_stk_show_ch0_state, NULL);
static struct class_attribute class_attr_ch1_state =
    __ATTR(ch1_state, 0444, class_stk_show_ch1_state, NULL);
static struct class_attribute class_attr_ch2_state =
    __ATTR(ch2_state, 0444, class_stk_show_ch2_state, NULL);
static struct class_attribute class_attr_ch3_state =
    __ATTR(ch3_state, 0444, class_stk_show_ch3_state, NULL);
...
static struct class_attribute class_attr_anfr_status =
    __ATTR(anfr_status, 0664, anfr_show, anfr_store);
static struct class_attribute class_attr_debug =
    __ATTR(debug, 0664, stk_debug_show, stk_debug_store);
...
static struct attribute *capsense_class_attrs[] =
{
	...
	&class_attr_ch0_state.attr,
	&class_attr_ch1_state.attr,
	&class_attr_ch2_state.attr,
    &class_attr_ch3_state.attr,
    ...
    for (i = 0; i < ch_num; i ++)
    {
        if(!strcmp(temp_input_dev->name, input_dev_name[i]))
        {
            ...
            if(en)
            {
                ...
                stk->last_prox_b_state = 0;
#endif
                input_report_rel(temp_input_dev, REL_MISC, SAR_STATE_FAR);
                input_sync(temp_input_dev);
...
static int32_t stk_init_mtk(stk501xx_wrapper *stk_wrapper)
{
    ...
    if (sensors_meta_input_init())
        STK_ERR("sensors_meta_input_init error!");
    ...
    for (i = 0; i < ch_num; i++)
    {
        err = sysfs_create_group(&stk_wrapper->channels[i].input_dev->dev.kobj,
                             &stk501xx_attr_group);
        stk_wrapper->channels[i].enabled = true;
    }
    ...
    if(!sar_sensor_class){
        STK_ERR("Failed to class_create");
    }
    ...
    if (ret)
    {
        STK_ERR("Failed to device_register:%d",ret);
    ...
    if (ret)
    {
        STK_ERR("Failed to device_register_wifi:%d",ret);
    ...
    if (ret)
    {
        STK_ERR("Failed to device_register_sub:%d",ret);
    ...
    if (ret)
    {
        STK_ERR("Failed to device_register_sub2:%d",ret);
...
#ifdef STK_WAKE_BY_IRQ
	device_init_wakeup(&stk_wrapper->i2c_mgr.client->dev, true);
    err = dev_pm_set_wake_irq(&stk_wrapper->i2c_mgr.client->dev, stk->gpio_info.irq);
    if (err < 0){
        STK_ERR("Failed to enable wake up gpio=%d irq=%d", stk->gpio_info.int_pin, stk->gpio_info.irq);
    }
#endif
