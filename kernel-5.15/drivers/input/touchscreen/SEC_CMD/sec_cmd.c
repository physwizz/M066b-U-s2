...
struct sec_cmd cmds[] = {
	{SEC_CMD("get_fw_ver_bin", NULL),},
	{SEC_CMD("get_fw_ver_ic", NULL),},
	{SEC_CMD("aot_enable", NULL),},
	{SEC_CMD("glove_mode", NULL),},
	{SEC_CMD("not_support_cmd", NULL),},
};
EXPORT_SYMBOL(cmds);
...
static void sec_cmd_store_function(struct sec_cmd_data *data)
{
	...
	if (cur) {
		memcpy(buff, buf, cur - buf);
		pr_err("%s %s:%d, buff=%s, cur=%s\n", SECLOG, __func__, __LINE__, buff, cur);
	} else {
		memcpy(buff, buf, len);
	}
	pr_debug("%s %s: COMMAND : %s\n", SECLOG, __func__, buff);
	...
	if (cmd_found) {
		pr_info("%s %s: cmd = %s", SECLOG, __func__, sec_cmd_ptr->cmd_name);
		if (!strncmp("aot_enable", sec_cmd_ptr->cmd_name, SEC_CMD_STR_LEN) && !param_cnt) {
			data->cmd_param[0] = 3;
		}
		for (i = 0; i < param_cnt; i++) {
			if (i == 0)
    ...
    pr_err("%s %s:%d,data->cmd=%s, sec_cmd_ptr->cmd_name=%s\n", 
                                SECLOG, __func__, __LINE__, data->cmd, sec_cmd_ptr->cmd_name);
    pr_err("%s %s:%d,cmd_param[0]=%d\n", SECLOG, __func__, __LINE__, data->cmd_param[0]);
	
	if (sec_cmd_ptr->cmd_func == NULL) {
		memset(buff, 0, sizeof(buff));
		sec_cmd_set_default_result(data);
		snprintf(buff, sizeof(buff), "%s", "tp loading fail");
		sec_cmd_set_cmd_result(data, buff, strnlen(buff, sizeof(buff)));
			...
			int len, int devt)
{
    ...
    pr_err("%s %s: CONFIG_DRV_SAMSUNG sec_device_create\n", SECLOG, __func__);
	data->fac_dev = sec_device_create(data, dev_name);
#endif
//	sec_class = class_create(THIS_MODULE, "sec");
...
static int __init sec_cmd_tp_init(void)
{
	int ret;

	pr_err("%s %s: ARRAY_SIZE(cmds)=%d\n", SECLOG, __func__, ARRAY_SIZE(cmds));

    ret = sec_cmd_init(&data, cmds, ARRAY_SIZE(cmds), SEC_CLASS_DEVT_TSP);
	if (ret < 0) {
		pr_err("%s %s: sec_cmd_init fail \n", SECLOG, __func__);
        return ret;
	}
	pr_err("%s %s: sec_cmd_tp_init Succeeded \n", SECLOG, __func__);
	return 0;
}

static void __exit sec_cmd_tp_exit(void)
{
	pr_err("%s %s: sec_cmd_tp_exit \n", SECLOG, __func__);
	sec_cmd_exit(&data, SEC_CLASS_DEVT_TSP);
	return;
}
module_init(sec_cmd_tp_init);
module_exit(sec_cmd_tp_exit);
