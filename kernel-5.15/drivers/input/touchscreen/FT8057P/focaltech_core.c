...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, events[i].p);
            input_report_abs(input_dev, ABS_MT_TOUCH_MINOR, events[i].area);
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_DEFAULT:
        ...
        for (i = 0; i < max_touch_num; i++) {
            ...
            events[i].p =  touch_buf[FTS_TOUCH_OFF_PRE + base];   //major
            events[i].area = touch_buf[FTS_TOUCH_OFF_AREA + base]; //minor
...
static int fts_ts_suspend(struct device *dev)
{
	...
	if (status) {
		FTS_INFO("in_black_test_suspend,status = %d ",status);
		fts_release_all_finger();
