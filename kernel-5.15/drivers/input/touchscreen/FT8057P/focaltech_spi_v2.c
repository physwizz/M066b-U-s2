...
int ft_lct_tp_selftest_callback(unsigned char cmd)
{
	int ret = 0;
	switch (cmd)
	{
	case TP_SELFTEST_CMD_LONGCHEER_MMI:
		ret = ft_lct_tp_selftest_all();
		break;
...
EXPORT_SYMBOL(ft_lct_tp_selftest_callback);
/****************sec_tsp class sysfs Feature start*****************/
static void get_fw_ver_bin(void *device_data)
{
    ...
    FTS_INFO("fw_ver=0x%02X", fwver);
    sec_cmd_set_default_result(sec);
    snprintf(buff, sizeof(buff), "0x%02X", fwver);
...
static void get_fw_ver_ic(void *device_data)
{
    ...
    sec_cmd_set_default_result(sec);
    snprintf(buff, sizeof(buff), "FT8057P");
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
        ...
        flag = 1;
        FTS_INFO("unknown sec->cmd_param[0] = %d\n", sec->cmd_param[0]);
    }
    double_click_flag = ts_data->gesture_support;
    FTS_INFO("double_click_flag = %d\n", double_click_flag);
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    lct_tp_selftest_init(ft_lct_tp_selftest_callback);
    tp_mode.charger_mode_switch_status = set_ft_tp_charger_state;
    /****************sec_tsp class sysfs Feature start*****************/
