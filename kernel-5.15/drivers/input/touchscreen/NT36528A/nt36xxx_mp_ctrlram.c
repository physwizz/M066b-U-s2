...
#define SHORT_TEST_CSV_FILE "/data/vendor/touchpad/ShortTest"
#define OPEN_TEST_CSV_FILE "/data/vendor/touchpad/OpenTest"
#define FW_RAWDATA_CSV_FILE "/data/vendor/touchpad/FWRawdataTest"
#define FW_CC_CSV_FILE "/data/vendor/touchpad/FWCCTest"
#define NOISE_TEST_CSV_FILE "/data/vendor/touchpad/NoiseTest"
#define PEN_FW_RAW_TEST_CSV_FILE "/data/vendor/touchpad/PenFWRawTest"
#define PEN_NOISE_TEST_CSV_FILE "/data/vendor/touchpad/PenNoiseTest"

static char mpFile_short[128];
static char mpFile_open[128];
static char mpFile_rawdata[128];
static char mpFile_cc[128];
static char mpFile_noise[128];
...
*******************************************************/
static int32_t RawDataTest_SinglePoint_Sub(int32_t rawdata[], uint8_t RecordResult[], uint8_t x_ch, uint8_t y_ch, int32_t Rawdata_Limit_Postive[], int32_t Rawdata_Limit_Negative[], char *file_path, uint32_t offset)
{
	int32_t i = 0;
	int32_t j = 0;
#if TOUCH_KEY_NUM > 0
...
#if NVT_SAVE_TEST_DATA_IN_FILE
		if(offset == 0) {
			//sprintf(file_path, "%s_FAIL.csv",file_path);
			strcat(file_path, "_FAIL.csv");
		}
#endif
		ret = -1; // FAIL
	} else {
#if NVT_SAVE_TEST_DATA_IN_FILE
		if(offset == 0) {
			//sprintf(file_path, "%s_PASS.csv",file_path);
			strcat(file_path, "_PASS.csv");
		}
#endif
		ret =  0; // PASS
	}
#if NVT_SAVE_TEST_DATA_IN_FILE
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(rawdata, x_ch, y_ch, file_path, offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		ret =  -EAGAIN;
	}
#endif
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if (mutex_lock_interruptible(&ts->lock)) {
	 ...
	 sprintf(mpFile_short, "%s_%04d%02d%02d_%02d%02d%02d", SHORT_TEST_CSV_FILE, 1900 + tm.tm_year,
            1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	 sprintf(mpFile_open, "%s_%04d%02d%02d_%02d%02d%02d", OPEN_TEST_CSV_FILE, 1900 + tm.tm_year,
            1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	 sprintf(mpFile_rawdata, "%s_%04d%02d%02d_%02d%02d%02d", FW_RAWDATA_CSV_FILE, 1900 + tm.tm_year,
            1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	 sprintf(mpFile_cc, "%s_%04d%02d%02d_%02d%02d%02d", FW_CC_CSV_FILE, 1900 + tm.tm_year,
            1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	 sprintf(mpFile_noise, "%s_%04d%02d%02d_%02d%02d%02d", NOISE_TEST_CSV_FILE, 1900 + tm.tm_year,
            1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
#endif
	...
	if (nvt_read_baseline(RawData_FW_Rawdata) != 0) {
		...
		TestResult_FW_Rawdata = RawDataTest_SinglePoint_Sub(RawData_FW_Rawdata, RecordResult_FW_Rawdata, X_Channel, Y_Channel,
												PS_Config_Lmt_FW_Rawdata_P, PS_Config_Lmt_FW_Rawdata_N, mpFile_rawdata, 0);
	}
	...
	if (nvt_read_CC(RawData_FW_CC) != 0) {
		...
		TestResult_FW_CC = RawDataTest_SinglePoint_Sub(RawData_FW_CC, RecordResult_FW_CC, X_Channel, Y_Channel,
											PS_Config_Lmt_FW_CC_P, PS_Config_Lmt_FW_CC_N, mpFile_cc, 0);
	}
	...
	if (nvt_read_fw_noise(RawData_Diff) != 0) {
		...
		TestResult_FW_DiffMax = RawDataTest_SinglePoint_Sub(RawData_Diff_Max, RecordResult_FW_DiffMax, X_Channel, Y_Channel,
											PS_Config_Lmt_FW_Diff_P, PS_Config_Lmt_FW_Diff_N, mpFile_noise, csv_output_offset);
#if TOUCH_KEY_NUM > 0
		csv_output_offset = Y_Channel * X_Channel * 7 + Y_Channel * 2 + Key_Channel * 7 + 2;
#else
		csv_output_offset = Y_Channel * X_Channel * 7 + Y_Channel * 2;
#endif /* #if TOUCH_KEY_NUM > 0 */
	...
	if (nvt_read_fw_short(RawData_Short) != 0) {
		...
		TestResult_Short = RawDataTest_SinglePoint_Sub(RawData_Short, RecordResult_Short, X_Channel, Y_Channel,
										PS_Config_Lmt_Short_Rawdata_P, PS_Config_Lmt_Short_Rawdata_N, mpFile_short, 0);
	}
	...
	if (nvt_read_fw_open(RawData_Open) != 0) {
		...
		TestResult_Open = RawDataTest_SinglePoint_Sub(RawData_Open, RecordResult_Open, X_Channel, Y_Channel,
											PS_Config_Lmt_Open_Rawdata_P, PS_Config_Lmt_Open_Rawdata_N,mpFile_open, 0);
	}
...
int novatek_lct_tp_selftest_all(void)
{
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	//novatek-mp-criteria-default
#if NVT_SAVE_TEST_DATA_IN_FILE
	...
	if ((TestResult_Short == 0) && (TestResult_Open == 0) && (TestResult_FW_Rawdata == 0) && (TestResult_FW_CC == 0) && (TestResult_Noise == 0)) {
		NVT_LOG("%s %d: novatek TP selftest pass, return 2\n", __func__, __LINE__);
		return 2; //pass
	} else {
		NVT_LOG("%s %d: novatek TP selftest fail, return 1 \n", __func__, __LINE__);
		return 1; //pass
	}
