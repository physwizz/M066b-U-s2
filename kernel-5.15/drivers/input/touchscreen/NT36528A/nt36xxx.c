...
#include "nt36xxx.h"
#include "../LCT_TP/lct_tp_driver.h"
...
extern struct touch_panel tp_mode;
...
info_retry:
	...
	NVT_LOG("[Vendor]TXD,[FW]0x%02X,[IC]nt36528a\n", ts->fw_ver);
	sprintf(tp_info_buf, "[Vendor]TXD,[FW]0x%02X,[IC]nt36528a\n", ts->fw_ver );
...
*/
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		status = (uint8_t)(point_data[position + 0] & 0x07);
		if ((input_id == 0) || (input_id > ts->max_touch_num))
			continue;

		//if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
		if ((status == FINGER_MOVING) || (status == PALM_TOUCH)) {
#if NVT_TOUCH_ESD_PROTECT
			/* update interrupt timer */
			irq_timer = jiffies;
#endif /* #if NVT_TOUCH_ESD_PROTECT */
			...
			if (input_p == 0) {
				input_p = 1;
				w_minor = (uint32_t)(point_data[i + 99]);
			}
			if (w_minor == 0)
				w_minor = 1;
...
static int nvt_charger_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
	...
	if ((!strcmp(psy->desc->name, "charger")) || (!strcmp(psy->desc->name, "usb"))) {
...
static void get_fw_ver_bin(void *device_data)
{
    ...
    snprintf(buff, sizeof(buff), "0x%02X", ts->fw_ver);
...
static void get_fw_ver_ic(void *device_data)
{
    ...
    sec_cmd_set_default_result(sec);
    snprintf(buff, sizeof(buff), "NT36528A");
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
        ...
        flag = 1;
        NVT_LOG("unknown sec->cmd_param[0] = %d\n", sec->cmd_param[0]);
    }
    double_click_flag = ts->gesture;
    NVT_LOG("double_click_flag = %d\n", double_click_flag);
...
int novatek_lct_tp_selftest_callback(unsigned char cmd)
{
	int ret = 0;
	switch (cmd)
	{
	case TP_SELFTEST_CMD_LONGCHEER_MMI:
		ret = novatek_lct_tp_selftest_all();
		break;
...
EXPORT_SYMBOL(novatek_lct_tp_selftest_callback);
...
static int32_t __init nvt_driver_init(void)
{
	...
	lct_tp_selftest_init(novatek_lct_tp_selftest_callback);
