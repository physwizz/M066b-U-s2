			...
			if (len < SRAM_TEST_SETTING_LEN) {
				for (i = 0; i < len; i++) {
					...
					if (ilits->cascade_info_block.nNum != 0) {
						ili_ice_mode_read_by_mode(ilits->sram_para.read_addr[i], &Bist_Sram_Fail, 4, MASTER);
						if ((Bist_Sram_Fail & SramRsultStatus[i] & mask) > 0) {
							ILI_ERR("Check Master Bist Fail Flag , Addr[%d] = 0x%X : Fail!\n", i, ilits->sram_para.read_addr[i]);
						...
						ili_ice_mode_read_by_mode(ilits->sram_para.read_addr[i], &Bist_Sram_Fail, 4, SLAVE);
						if ((Bist_Sram_Fail & SramRsultStatus[i] & mask) > 0) {
							ILI_ERR("Check Slave Bist Fail Flag , Addr[%d] = 0x%X : Fail!\n", i, ilits->sram_para.read_addr[i]);
							ILI_ERR("Bist Sram Fail Status = 0x%X, Sram Rsult Status = 0x%X, mask = 0x%X\n", Bist_Sram_Fail, SramRsultStatus[i], mask);
							ILI_ERR("Check Bist_Fail_Flag Fail!!,FailValue= = 0x%X\n", Bist_Sram_Fail & SramRsultStatus[i] & mask);
							ret = -1;
						}
...
out:
    sprintf(tp_info_buf, "[Vendor]HKC,[FW]%d.%d.%d.%d,[IC]ILI79505\n", buf[1], buf[2], buf[3], buf[4]);
