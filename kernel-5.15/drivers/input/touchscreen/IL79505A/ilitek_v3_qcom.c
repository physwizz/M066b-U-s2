...
int lct_ili_tp_selftest_callback(unsigned char cmd)
{
	int ret = 0;
	switch (cmd)
	{
	case TP_SELFTEST_CMD_LONGCHEER_MMI:
		ret = ili_lct_tp_selftest_all();
		break;
...
EXPORT_SYMBOL(lct_ili_tp_selftest_callback);
...
static void get_fw_ver_bin(void *device_data)
{
    ...
    ILI_INFO(" fw_ver=0x%02X\n", (ilits->chip->fw_ver >> 8) & 0xFF);
    sec_cmd_set_default_result(sec);
    snprintf(buff, sizeof(buff), "0x%02X", (ilits->chip->fw_ver >> 8) & 0xFF);
...
static void get_fw_ver_ic(void *device_data)
{
    ...
    ILI_INFO(" ilits->md_name=%s\n", ilits->md_name);
    sec_cmd_set_default_result(sec);
    snprintf(buff, sizeof(buff), "%s", ilits->md_name);
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
        ...
        flag = 1;
        ILI_INFO("unknown sec->cmd_param[0] = %d\n", sec->cmd_param[0]);
    }
...
static int ilitek_plat_probe(void)
{
	...
	lct_tp_selftest_init(lct_ili_tp_selftest_callback);
#if SPRD_SYSFS_SUSPEND_RESUME
