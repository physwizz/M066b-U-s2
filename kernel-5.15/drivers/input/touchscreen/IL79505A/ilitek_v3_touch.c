...
int ili_touch_esd_gesture_flash(void)
{
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		mdelay(90);/*Waiting for load geasture by Darcy*/
	} else {
		ret = ili_reset_ctrl(ilits->reset);
		mdelay(20);/*Waiting for load geasture by Darcy*/
	}
...
static int ili_spi_write_ges_password(int pwd_adr, int pwd, int pwd_len)
{
	...
	/*Slave*/
	if (ilits->cascade_info_block.nNum != 0) {
		ili_spi_ice_mode_read(pwd_adr, &answer, sizeof(u32), SLAVE);
	...
	/*Master*/
	answer = 0;
	ili_spi_ice_mode_read(pwd_adr, &answer, sizeof(u32), MASTER);
...
static void ilitek_tddi_touch_customer_data_parsing(u8 *buf)
{
	...
	ilits->finger = 0;
#if AXIS_PACKET
	memset(ilits->axis_info, 0x0, sizeof(ilits->axis_info));
#endif
	if (ilits->rib.nCustomerType == POSITION_CUSTOMER_TYPE_AXIS && ilits->tp_data_format == DATA_FORMAT_DEMO) {
		...
		#if AXIS_PACKET
				ilits->axis_info[ilits->finger].degree = axis_info[ilits->finger].degree;
				ilits->axis_info[ilits->finger].width_major = axis_info[ilits->finger].width_major;
				ilits->axis_info[ilits->finger].width_minor = axis_info[ilits->finger].width_minor;
		#endif
...
#if AXIS_PACKET
		input_report_abs(ilits->input, ABS_MT_TOUCH_MAJOR, ilits->axis_info[id].width_major);
		input_report_abs(ilits->input, ABS_MT_TOUCH_MINOR, ilits->axis_info[id].width_minor);
#endif
...
static void ili_debug_mode_report_point(u8 *buf, int len)
{
	int i = 0;
	u32 xop = 0, yop = 0;
	static u8 p[MAX_TOUCH_NUM];
