...
#include "../../power/supply/nopmi_mtk_battery_name.h"
...
static const int master_charger_state_to_current_limit[CHARGER_STATE_NUM] = {
	-1, 4000000, 3500000, 3000000, 2500000, 2000000, 1500000, 1000000, 500000, 0
};
...
static int cooling_state_to_charger_limit_v1(struct charger_cooling_device *chg)
{
	...
	prop_bat_chr.intval = chg->target_state;

	ret = power_supply_set_property(chg->chg_psy,
		POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT,
		&prop_bat_chr);
	if (ret != 0) {
		pr_err("%s set bat thermal level fail\n", __func__);
		return ret;
	}
...
static int charger_cooling_probe(struct platform_device *pdev)
{
	...
	charger_cdev->chg_psy = power_supply_get_by_name(NOPMI_BATTERY_PSY_NAME);
