...
static int gadc_thermal_adc_to_temp(struct gadc_thermal_info *gti, int val)
{
	...
	if (i == 0) {
		...
		temp = temp_hi + mult_frac(temp_lo - temp_hi, val - adc_hi,
					   adc_lo - adc_hi);
	    printk("Thermal gadc_thermal_info 2  temp =%d,temp_hi =%d ,temp_lo %d adc_hi =%d adc_lo =%d temp = %d \n",temp,temp_hi,temp_lo,adc_hi,adc_lo,temp);
	}
	printk("Thermal gadc_thermal_info 2  temp =%d,t\n",temp);
	return temp;
}
...
static int gadc_thermal_get_temp(void *data, int *temp)
{
	...
	printk("Thermal gadc_thermal_get_temp  1 val =%d,ret =%d ,gti->channel->number %d \n",val,ret,gti->channel->data);
	if (ret < 0) {
		dev_err(gti->dev, "IIO channel read failed %d\n", ret);
		return ret;
	}
	*temp = gadc_thermal_adc_to_temp(gti, val);
	printk("Thermal gadc_thermal_get_temp 2  val =%d,ret =%d ,gti->channel->number %d temp = %d \n",val,ret,gti->channel->data,&temp);
						 ...
						 struct gadc_thermal_info *gti)
{
	...
	int ntable;
	int ret;
	printk("Thermal gadc_thermal_read_linear_lookup_table\n");
