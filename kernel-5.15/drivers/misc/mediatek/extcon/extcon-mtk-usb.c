							...
							bool is_on)
{
	...
	if (!vbus || extcon->vbus_on == is_on)
		return 0;
	chg_dev = get_charger_by_name("charger");
  	if(!chg_dev)
          dev_info(dev, "can`t get upm6722_chg\n");
	...
	if (is_on) {
		...
		dev_info(dev, "charger_dev_enable_otg enable\n");
		...
		charger_dev_enable_otg(chg_dev, false);
          	dev_info(dev, "charger_dev_enable_otg disable\n");
	}
