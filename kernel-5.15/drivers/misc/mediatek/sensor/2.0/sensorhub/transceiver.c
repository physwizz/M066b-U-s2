...
static bool transceiver_wakeup_check(uint8_t action, uint8_t sensor_type)
{
	...
	if (action == DATA_ACTION && (sensor_type == SENSOR_TYPE_PROXIMITY ||
			...
			sensor_type == SENSOR_TYPE_SMART_ALERT ||
...
static int transceiver_calibration_leak(struct hf_device *hf_dev,
        int sensor_type)
{
    printk("transceiver_calibration leak\n");
    return transceiver_comm_with(sensor_type,
        SENS_COMM_CTRL_CALI_LEAK_CMD, NULL, 0);
}
...
static int __init transceiver_init(void)
{
	...
	dev->hf_dev.leak_calibrate = transceiver_calibration_leak;
