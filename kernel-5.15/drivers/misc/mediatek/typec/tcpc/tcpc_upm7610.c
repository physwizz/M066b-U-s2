...
static inline upm7610_init_cfg_adjust(struct tcpc_device *tcpc);
...
static int upm7610_init_alert(struct tcpc_device *tcpc)
{
	...
	enable_irq_wake(chip->irq);// need to test this line on huaqin VD7335A 
	return 0;
}
...
static inline int upm7610_check_revision(struct i2c_client *client)
{
	...
	pr_info("%s successful, PID=0x%04x, VID=0x%04x\n", __func__, pid, vid);
	return did;
}
