	...
	struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		if (old_uisoc != gm->ui_soc) {
			...
			bm_err("[K]FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d %d GM3:%d old:%d diff=%ld\n",
			...
			bm_debug("[K]FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d %d GM3:%d\n",
	...
	case FG_DAEMON_CMD_SET_SOC:
	{
		...
		gm->precise_soc = (int_value + 5) / 10;
		bm_debug("[K]FG_DAEMON_CMD_SET_SOC %d, %d\n",
			gm->soc, gm->precise_soc);
	}
	break;
