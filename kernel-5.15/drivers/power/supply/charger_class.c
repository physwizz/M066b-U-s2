...
int charger_dev_enable_hiz(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->enable_hiz)
		return chg_dev->ops->enable_hiz(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_hiz);
