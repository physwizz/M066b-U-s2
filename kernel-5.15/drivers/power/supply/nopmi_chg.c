...
#define CYCLE_COUNT_STEP1 200
#define CYCLE_COUNT_STEP2 250
#define CYCLE_COUNT_STEP3 300
#define CYCLE_COUNT_STEP4 1001
#define FLOAT_VOLTAGE_DEFAULT 4456
#define FLOAT_VOLTAGE_STEP1   4441
#define FLOAT_VOLTAGE_STEP2   4421
#define FLOAT_VOLTAGE_STEP3   4401
#define FLOAT_VOLTAGE_STEP4   4351
...
#define PPS_CHARGE_CURRENT 3200
#define AFC_QC_PD_CHARGE_CURRENT 2700
...
static ssize_t usb_real_type_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int val = 0 ;
    struct nopmi_chg *chg = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RO(usb_real_type);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (chg->pd_active || (quick_chgtype == POWER_SUPPLY_TYPE_USB_PD)) {
		...
		if(!chg->main_psy)
			chg->main_psy = power_supply_get_by_name("charger");
		if (chg->main_psy) {
...
static DEVICE_ATTR_RW(mtbf_current);

/*longcheer nielianjie10 2022.12.05 Set CV according to circle count start*/
static ssize_t cycle_count_select_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct nopmi_chg *chg = dev_get_drvdata(dev);
...
static ssize_t cycle_count_select_store(struct device *dev,
		struct device_attribute *attr,const char *buf, size_t len)
{
	struct nopmi_chg *chg = dev_get_drvdata(dev);
    int val = 0;
...
static DEVICE_ATTR_RW(cycle_count_select);
...
static ssize_t fg_batt_id_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
	struct nopmi_chg *chg = dev_get_drvdata(dev);
        int rc = 0;
...
static DEVICE_ATTR_RO(fg_batt_id);
...
static DEVICE_ATTR_RO(typec_mode);
...
static DEVICE_ATTR(afc_disable, 0660, charge_afc_show, charge_afc_store);
...
static struct attribute *battery_attributes[] = {
	...
	//
	&dev_attr_usb_real_type.attr,
	&dev_attr_real_type.attr,
	&dev_attr_typec_mode.attr,
	&dev_attr_typec_cc_orientation.attr,
	&dev_attr_resistance_id.attr,
	&dev_attr_input_suspend.attr,
	&dev_attr_quick_charge_type.attr,
	&dev_attr_mtbf_current.attr,
	&dev_attr_thermal_level.attr,
	&dev_attr_fg_batt_id.attr,
	&dev_attr_cycle_count_select.attr,
...
static int nopmi_chg_init_dev_class(struct nopmi_chg *chg)
{
	...
	chg->sec_dev = sec_device_create(chg, "switch");
	if (IS_ERR_OR_NULL(chg->sec_dev)) {
		pr_err("failed to create sec_dev device\n");
		return rc;
	}

	rc = sysfs_create_groups(&chg->sec_dev->kobj, battery_attr_groups);
	if (rc < 0) {
		pr_err("failed to create sec_dev sysfs\n");
	}
...
static const char * const *Get_Bms_Info(struct nopmi_chg *chip)
{
	...
	{
		chip->bms_psy = power_supply_get_by_name(MTK_BATTERY_PSY_NAME);
		if(chip->bms_psy == NULL) {
...
static const char * const *Get_Chg_Info(struct nopmi_chg *chip)
{
	...
	{
		chip->bbc_psy = power_supply_get_by_name("charger");
		if(chip->bbc_psy == NULL) {
				...
				const int value)
{
	...
	if (value == 0) {
		...
		rc = nopmi_chg_get_iio_channel(chip, NOPMI_CP, CHARGE_PUMP_SP_BUS_VOLTAGE, &vbus);
		if (rc < 0)
			pr_err("%s get cp vbus fail\n", __func__);
	...
	schedule_delayed_work(&chip->fv_vote_monitor_work,
		msecs_to_jiffies(10));
				...
				const int value)
{
	...
	if (value) {
		...
		if ((nopmi_chg->batt_slate_mode == 1) || (nopmi_chg->batt_slate_mode == 2)
		    || (soc_full == 1 && ((nopmi_chg->batt_full_capacity_soc != 0) && (nopmi_chg->batt_full_capacity_soc != 100)))
...
#endif
#ifdef CONFIG_LCT_FACTORY_VERSION
#endif
...
/*#ifdef NOPMI_FULL_IN_ADVANCE
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (pval->intval >= 80) {
			...
			pr_info("uisoc %d between 50% and 70%\n", pval->intval);
#else
		pr_info("Did't in factory mode! CONFIG_LCT_FACTORY_VERSION not define\n");
#endif
		break;
...
static int nopmi_charge_control_limit(struct nopmi_chg *nopmi_chg, const union power_supply_propval *val)
{
	int rc = 0;
	union power_supply_propval pval = {0, };
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
	#ifdef CONFIG_LCT_FACTORY_VERSION
		nopmi_chg->raw_thermal_level = val->intval;
		if(nopmi_chg->thermal_disable) {
			pr_err("thermal_disabling, raw_thermal_level=%d,%d\n", val->intval, nopmi_chg->raw_thermal_level);
			break;
		}
	#endif
		rc = nopmi_charge_control_limit(nopmi_chg, val);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		power_supply_set_property(nopmi_chg->bms_psy, prop, val);
		break;
...
#ifdef CONFIG_LCT_FACTORY_VERSION
static int thermal_disable(struct nopmi_chg *nopmi_chg, bool disable)
{
	union power_supply_propval val = {0, };

	pr_info("disable=%d thermal_disable=%d raw_thermal_level=%d system_temp_level=%d\n", disable, nopmi_chg->thermal_disable, nopmi_chg->raw_thermal_level, nopmi_chg->system_temp_level);
	if(disable) {
		nopmi_chg->thermal_disable = 1;
		val.intval = 0;
		nopmi_charge_control_limit(nopmi_chg, &val);
	} else {
		nopmi_chg->thermal_disable = 0;
		val.intval = nopmi_chg->raw_thermal_level;
		nopmi_charge_control_limit(nopmi_chg, &val);
	}
	pr_info("thermal_disable=%d raw_thermal_level=%d system_temp_level=%d\n", nopmi_chg->thermal_disable, nopmi_chg->raw_thermal_level, nopmi_chg->system_temp_level);

	return 0;
}

static ssize_t charge_test_show(struct device *dev,
          struct device_attribute *attr,
          char *buf)
{
	struct nopmi_chg *nopmi_chg = container_of(attr, struct nopmi_chg , attr_charge_test);

	pr_info("charge_test=%d thermal_disable=%d raw_thermal_level=%d system_temp_level=%d\n", nopmi_chg->charge_test, nopmi_chg->thermal_disable, nopmi_chg->raw_thermal_level, nopmi_chg->system_temp_level);
	return sprintf(buf,"%d\n",nopmi_chg->charge_test);
}

static ssize_t charge_test_store(struct device *dev,
           struct device_attribute *attr,
           const char *buf, size_t count)
{
	int ret = 0;
	...
	if(ret < 0)
		return -EINVAL;
	nopmi_chg->charge_test = val;
	pr_info("charge_test=%d\n", nopmi_chg->charge_test);
	thermal_disable(nopmi_chg, val);
          ...
          char *buf)
{
	...
	rc = power_supply_get_property(chg->main_psy, POWER_SUPPLY_PROP_USB_TYPE , &pval);
	...
	rc = power_supply_get_property(chg->main_psy, POWER_SUPPLY_PROP_ONLINE, &pval2);
	if (rc < 0) {
		pr_err("couldn't readusb online roperty, ret=%d\n", rc);
		return -EINVAL;
	}
	chg->usb_online  = pval2.intval;
	pr_err("val2 intval:%d\n",pval2.intval);
#if 0
	...
	if((pval.intval ==  POWER_SUPPLY_USB_TYPE_UNKNOWN) && chg->usb_online && !pd_active) {
		chg->batt_misc_event |= BATT_MISC_EVENT_TIMEOUT_OPEN_TYPE;
	} else {
		chg->batt_misc_event &= ~BATT_MISC_EVENT_TIMEOUT_OPEN_TYPE;
	}

	if ((soc_full == 1) && (chg->input_suspend == 1) && pval2.intval) {
		chg->batt_misc_event |= BATT_MISC_EVENT_FULLCAPACITY_TYPE;
	} else {
		chg->batt_misc_event &= ~BATT_MISC_EVENT_FULLCAPACITY_TYPE;
	} 
	pr_err("batt_misc_event:%x,soc_full=%d,input_suspend=%d,usb_online=%d,batt_soc_rechg=%d\n", chg->batt_misc_event, soc_full, chg->input_suspend, chg->usb_online, chg->batt_soc_rechg);
	...
	char *buf)
{
	...
	if (chg->usb_online == 0) {
		...
		if (!chg->main_psy)
			chg->main_psy = power_supply_get_by_name("charger");
		if (chg->main_psy) {
			ret = power_supply_get_property(chg->main_psy, POWER_SUPPLY_PROP_USB_TYPE, &pval);
		}
		if (ret < 0) {
			charger_type = POWER_SUPPLY_USB_TYPE_UNKNOWN;
			pr_err("%s : get POWER_SUPPLY_PROP_CHARGE_TYPE fail\n", __func__);
		} else {
			charger_type = pval.intval;
			pr_err("POWER_SUPPLY_PROP_CHARGE_TYPE charger_type: %d\n", charger_type);
		}
	...
	if ((chg->batt_full_capacity_soc == 80 )||(chg->batt_full_capacity_soc == 85)||(chg->batt_full_capacity_soc == 90)||(chg->batt_full_capacity_soc == 95)) {
		batt_fcc = batt_fcc * chg->batt_full_capacity_soc/100;
	}
           ...
           const char *buf, size_t count)
{
	...
	if (chg->batt_psy == NULL) {
		chg->batt_psy = power_supply_get_by_name(MTK_BATTERY_PSY_NAME);
	}
	if (chg->batt_psy) {
...
static int nopmi_init_batt_psy(struct nopmi_chg *chg)
{
	...
	battery_sysfs_create_group(chg->batt_psy);
...
#ifdef CONFIG_LCT_FACTORY_VERSION
	sysfs_attr_init(&chg->attr_charge_test.attr);
	chg->attr_charge_test.attr.name = "charge_test";
	chg->attr_charge_test.attr.mode = 0666;
	chg->attr_charge_test.show = charge_test_show;
	chg->attr_charge_test.store = charge_test_store;
	rc = sysfs_create_file(&chg->batt_psy->dev.kobj,&chg->attr_charge_test.attr);
	if(rc)
		return -EINVAL;
#endif
	...
	sysfs_attr_init(&chg->attr_batt_slate_mode.attr);
...
{
	struct nopmi_chg *nopmi_chg = power_supply_get_drvdata(psy);
	union power_supply_propval bc12_type = {0, };
	int rc = 0;
	switch (psp) {
		case POWER_SUPPLY_PROP_ONLINE:
#if 0
			if(!nopmi_chg->main_psy)
				nopmi_chg->main_psy = power_supply_get_by_name("charger");
			if (nopmi_chg->main_psy) {
...
#endif
			if(IS_ERR_OR_NULL(nopmi_chg->main_psy)) {
				nopmi_chg->main_psy = power_supply_get_by_name("charger");
			}

			if (!IS_ERR_OR_NULL(nopmi_chg->main_psy)) {
				rc = power_supply_get_property(nopmi_chg->main_psy, POWER_SUPPLY_PROP_USB_TYPE, &bc12_type);
				if (rc < 0) {
					pr_err("%s : get POWER_SUPPLY_PROP_USB_TYPE fail\n", __func__);
				}
			}
			if (bc12_type.intval == POWER_SUPPLY_USB_TYPE_DCP)
				pval->intval = g_usb_online;
			else
				pval->intval = false;
			break;
		default:
		...
		union power_supply_propval *val)
{
		...
		case POWER_SUPPLY_PROP_ONLINE:
			...
			if (nopmi_chg->main_psy) {
				...
				if (rc < 0) {
					...
					val->intval = value.intval;
					pr_info("%s: POWER_SUPPLY_PROP_ONLINE val->intval: %d\n", __func__, val->intval);
				}
			}

			g_usb_online = val->intval;
			break;
	...
	pr_info("%s: usb_online: %d, last_online:%d \n", __func__, g_usb_online, last_usb_online);
	if(g_usb_online != last_usb_online){
		last_usb_online = g_usb_online;
		power_supply_changed(nopmi_chg->usb_psy);
		power_supply_changed(nopmi_chg->batt_psy);
		pr_info("%s: usb_online changed !!! notify \n", __func__, g_usb_online, last_usb_online);
        }
	return ret;
}
...
static int phone_c_to_c_source_cap(struct tcpc_device *tcpc)
{
	int ret, i;
	struct tcpm_power_cap cap = {0};
	struct tcpm_power_cap_val apdo_cap = {0};
	int icl = TCPM_SUCCESS;

	ret = tcpm_inquire_pd_source_cap(tcpc, &cap);
	if (ret != TCPM_SUCCESS) {
		pr_err("Failed to inquire PD source cap: %d\n", ret);
		return ret;
	}

	if (cap.cnt == 0) {
		pr_err("No source capabilities available\n");
		return TCPM_ERROR_NO_SOURCE_CAP;
	}

	for (i = cap.cnt - 1; i >= 0; i--) {
		tcpm_extract_power_cap_val(cap.pdos[i], &apdo_cap);

		if (apdo_cap.type == TCPM_POWER_CAP_VAL_TYPE_FIXED) {
			if (apdo_cap.min_mv >= 9000) {
				icl = min(apdo_cap.ma, 1670);
				break;
			} else if (apdo_cap.min_mv <= 5000) {
				icl = min(apdo_cap.ma, 1500);
			}
		}
	}
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_PRESENT:
			pr_notice("adapter/usb present %d\n", val->intval);
			...
			vote(nopmi_chg->usb_icl_votable, OPS_SET_VOTER, false, 0);
			if(val->intval) {
				int pd_type=POWER_SUPPLY_USB_TYPE_UNKNOWN, bc12_type=POWER_SUPPLY_USB_TYPE_UNKNOWN;
				int charger_type=POWER_SUPPLY_USB_TYPE_UNKNOWN;
				int icl = 500;
				int fcc = VERIFY_BAT;

				vote(nopmi_chg->usb_icl_votable, DCP_CHG_VOTER, false, 0);
				vote(nopmi_chg->usb_icl_votable, PD_CHG_VOTER, false, 0);
				vote(nopmi_chg->fcc_votable, DCP_CHG_VOTER, false, 0);
				...
				if (!nopmi_chg->tcpc_dev)
					...
					rc = tcpm_get_pd_connect_type(nopmi_chg->tcpc_dev);
					if ((rc == PD_CONNECT_PE_READY_SNK_PD30) || (rc == PD_CONNECT_PE_READY_SNK)) {
						pd_type = POWER_SUPPLY_USB_TYPE_PD;
					} else if (rc == PD_CONNECT_PE_READY_SNK_APDO) {
						pd_type = POWER_SUPPLY_USB_TYPE_PD_PPS;
					}
					pr_info("pd is %d, pd_type=%d\n", rc, pd_type);
				}

				if(!nopmi_chg->main_psy)
					nopmi_chg->main_psy = power_supply_get_by_name("charger");
				power_supply_get_property(nopmi_chg->main_psy, POWER_SUPPLY_PROP_USB_TYPE, &pval);
				bc12_type = pval.intval;

				if(pd_type!=POWER_SUPPLY_USB_TYPE_UNKNOWN) {
					charger_type = pd_type;
				} else {
					charger_type = pval.intval;
				}
				nopmi_chg->charger_type = charger_type;
				...
				switch (charger_type)
				{
					...
					case POWER_SUPPLY_USB_TYPE_PD:
						ret = phone_c_to_c_source_cap(nopmi_chg->tcpc_dev);
						if (ret > 0)
							icl = ret;
						else
							icl = 500;
					...
					case POWER_SUPPLY_USB_TYPE_PD_PPS:
						ret = phone_c_to_c_source_cap(nopmi_chg->tcpc_dev);
						if(bc12_type == POWER_SUPPLY_USB_TYPE_SDP) {
							if (ret > 0)
								icl = ret;
							else
								icl = 500;
				...
				vote(nopmi_chg->usb_icl_votable, CHARGER_TYPE_VOTER, true, icl);
				vote(nopmi_chg->fcc_votable, BAT_VERIFY_VOTER, true, fcc);
				pr_info("usb_online bc12_type=%d, pd_type=%d, charger_type=%d, icl=%dma, fcc=%dma successful !\n", bc12_type, pd_type, charger_type, icl, fcc);
			} else {
				vote(nopmi_chg->usb_icl_votable, DCP_CHG_VOTER, false, 0);
				vote(nopmi_chg->usb_icl_votable, PD_CHG_VOTER, false, 0);
				vote(nopmi_chg->fcc_votable, DCP_CHG_VOTER, false, 0);
				...
				vote(nopmi_chg->fcc_votable, AFC_CHG_DARK_VOTER, false, 0);
				vote(nopmi_chg->fcc_votable, BAT_VERIFY_VOTER, true, 500);
				vote(nopmi_chg->usb_icl_votable, CHARGER_TYPE_VOTER, true, 500);
			}
			break;
		case POWER_SUPPLY_PROP_ONLINE:
			nopmi_chg->usb_online = val->intval;
			g_usb_online = nopmi_chg->usb_online;
...
static char *nopmi_usb_supplied_to[] = { MTK_BATTERY_PSY_NAME, };
...
static int nopmi_parse_dt_jeita(struct nopmi_chg *chg, struct device_node *np)
{
	...
	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_p_cv", &val) >= 0)
		chg->jeita_ctl.dt.jeita_temp_t2_to_t3_p_cv = val;
	else {
		pr_err("use default JEITA_TEMP_T2_TO_T3_P_CV:%d\n",
		   TEMP_TN1_TO_T0_FCC);
		chg->jeita_ctl.dt.jeita_temp_t2_to_t3_p_cv = JEITA_TEMP_T2_TO_T3_P_CV;
	}
	if (of_property_read_u32(np, "temp_t2_to_t3_p_fcc", &val) >= 0)
		chg->jeita_ctl.dt.temp_t2_to_t3_p_fcc = val;
	else {
		pr_err("use default TEMP_T2_TO_T3_P_FCC:%d\n",
		   TEMP_TN1_TO_T0_FCC);
		chg->jeita_ctl.dt.temp_t2_to_t3_p_fcc = TEMP_T2_TO_T3_P_FCC;
	}
	return 0;
}
...
static void  nopmi_fv_vote_monitor_work(struct work_struct *work)
{
	...
	//pr_info("debug battery count :%d battery count :%d\n",debug_battery_cycle_count,battery_cycle_count);
...
static void nopmi_batt_capacity_check_work(struct work_struct *work)
{
	...
	nopmi_psy = power_supply_get_by_name(NOPMI_BATTERY_PSY_NAME);
	if(!nopmi_psy) {
		pr_err("%s power_supply_get_by_name bms fail !\n",__func__);
	}
	rc = power_supply_get_property(nopmi_psy, POWER_SUPPLY_PROP_CAPACITY,&pval);
	if (rc < 0) {
	...
	if(charge_protect_stop_charge != 0 && charge_protect_stop_charge != 100){
		...
		if(soc >= charge_protect_stop_charge) {
			...
			pr_err("start batt_full_capacity soc > CHARGE_PROTECT_STOP_CHARGE stop charge\n");
			...
			if(soc_full != 1) {
				...
				pr_err("first start batt_full_capacity soc < CHARGE_PROTECT_STOP_CHARGE start charge\n");
			}
		...
		if (nopmi_chg->batt_psy == NULL) {
			nopmi_chg->batt_psy = power_supply_get_by_name(MTK_BATTERY_PSY_NAME);
		}
		if (nopmi_chg->batt_psy) {
			power_supply_changed(nopmi_chg->batt_psy);
		}
	}else {
		if(nopmi_chg->batt_full_capacity_soc != batt_full_capacity_pre){
			soc_full = 0;
			batt_full_capacity_pre = nopmi_chg->batt_full_capacity_soc;
			pval.intval = 1;
			rc = power_supply_set_property(nopmi_chg->usb_psy, POWER_SUPPLY_PROP_ONLINE, &pval);
...
static void get_cp_type(struct nopmi_chg *chg)
{
	if (is_cp_chan_valid(chg, CHARGE_PUMP_SP_BUS_VOLTAGE)) {
		...
		pr_err("It's upm6720\n");
	} else if(is_cp_chan_valid(chg, (CHARGE_PUMP_SP_BUS_VOLTAGE + SC8541_IIO_CHANNEL_OFFSET))) {
		chg->cp_type = CP_SC8541;
		pr_err("It's sc8541\n");
	} else {
			...
			enum nopmi_chg_iio_type type, int channel, int *val)
{
	...
	case NOPMI_CP:
		...
		if (chg->cp_type == CP_SC8541) {
			cp_offset = SC8541_IIO_CHANNEL_OFFSET;
		} else if (chg->cp_type == CP_UPM6720) {
	...
	if (event == TCP_NOTIFY_TYPEC_STATE)
	{
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			noti->typec_state.new_state == TYPEC_ATTACHED_NORP_SRC)) {
				...
				g_nopmi_chg->typec_mode = typec_mode;
				g_nopmi_chg->cc_orientation = noti->typec_state.polarity;
				pr_err("%stypec mode = %d\n",__func__, g_nopmi_chg->typec_mode);
				pr_err("%scc_or_val = %d\n",__func__, g_nopmi_chg->cc_orientation);
				power_supply_changed(g_nopmi_chg->main_psy);
...
static int nopmi_chg_probe(struct platform_device *pdev)
{
	...
	main_psy = power_supply_get_by_name("charger");
	...
	for(i = 0;i < 20;i++) {
		bms_psy = power_supply_get_by_name(MTK_BATTERY_PSY_NAME);
...
//2021.09.21 wsy edit reomve vote to jeita
	...
	if (!nopmi_chg->tcpc_dev)
		...
		rc = tcpm_inquire_typec_attach_state(nopmi_chg->tcpc_dev);
		if (rc == TYPEC_UNATTACHED) {
			g_nopmi_chg->typec_mode = QTI_POWER_SUPPLY_TYPEC_NONE;
		} else {
			g_nopmi_chg->typec_mode = QTI_POWER_SUPPLY_TYPEC_SOURCE_DEFAULT;
			g_nopmi_chg->cc_orientation = tcpm_inquire_cc_polarity(nopmi_chg->tcpc_dev);
		}
	}
...
static int nopmi_chg_remove(struct platform_device *pdev)
{
	...
	if(nopmi_chg->sec_dev)
		sec_device_destroy(nopmi_chg->sec_dev->devt);
...
struct nopmi_mtk_battery_sysfs_field_info {
	struct device_attribute attr;
	enum battery_property prop;
};

#define NOPMI_BAT_SYSFS_FIELD_RW(_name, _prop)	\
{									 \
	.attr	= __ATTR(_name, 0644, bat_sysfs_show, bat_sysfs_store),\
	.prop	= _prop,	\
}

#define NOPMI_BAT_SYSFS_FIELD_RO(_name, _prop)	\
{			\
	.attr   = __ATTR(_name, 0444, bat_sysfs_show, bat_sysfs_store),\
	.prop   = _prop,				  \
}

#define NOPMI_BAT_SYSFS_FIELD_WO(_name, _prop)	\
{								   \
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct nopmi_mtk_battery_sysfs_field_info *nopmi_battery_attr;
	enum battery_property bp;
	struct device_attribute *mtk_battery_attr;
	...
	nopmi_battery_attr = container_of(attr,
	struct nopmi_mtk_battery_sysfs_field_info, attr);
	bp = nopmi_battery_attr->prop;
	if(IS_ERR_OR_NULL(gm->battery_sysfs_field_tbl)) {
		pr_err("mtk battery_sysfs_field_tbl no export\n");
		power_supply_put(psy);
		return PTR_ERR(gm->battery_sysfs_field_tbl);
	}
	mtk_battery_attr = &gm->battery_sysfs_field_tbl[bp].attr;

	pr_debug("MTK battery_property %d",bp);
	ret = mtk_battery_attr->show(&psy->dev, mtk_battery_attr, buf);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_debug("MTK battery_property %d",bp);
	ret = mtk_battery_attr->store(&psy->dev, mtk_battery_attr, buf, count);
...
static struct nopmi_mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	NOPMI_BAT_SYSFS_FIELD_RW(temperature, BAT_PROP_TEMPERATURE),
	NOPMI_BAT_SYSFS_FIELD_WO(coulomb_int_gap, BAT_PROP_COULOMB_INT_GAP),
	NOPMI_BAT_SYSFS_FIELD_WO(uisoc_ht_int_gap, BAT_PROP_UISOC_HT_INT_GAP),
	NOPMI_BAT_SYSFS_FIELD_WO(uisoc_lt_int_gap, BAT_PROP_UISOC_LT_INT_GAP),
	NOPMI_BAT_SYSFS_FIELD_WO(en_uisoc_ht_int, BAT_PROP_ENABLE_UISOC_HT_INT),
	NOPMI_BAT_SYSFS_FIELD_WO(en_uisoc_lt_int, BAT_PROP_ENABLE_UISOC_LT_INT),
	NOPMI_BAT_SYSFS_FIELD_WO(uisoc, BAT_PROP_UISOC),
	NOPMI_BAT_SYSFS_FIELD_RW(disable, BAT_PROP_DISABLE),
	NOPMI_BAT_SYSFS_FIELD_RW(init_done, BAT_PROP_INIT_DONE),
	NOPMI_BAT_SYSFS_FIELD_WO(reset, BAT_PROP_FG_RESET),
	NOPMI_BAT_SYSFS_FIELD_RW(log_level, BAT_PROP_LOG_LEVEL),
};
