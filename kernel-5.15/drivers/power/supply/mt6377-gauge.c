	...
	struct mtk_gauge_sysfs_field_info *attr, int *nafg_c_dltv)
{
	...
	*nafg_c_dltv = nag_c_dltv_mv_value;
	...
	bm_debug("[fg_bat_nafg][%s] mV:Reg[%d:%d] [b10:%d][26_16(0x%04x) 15_00(0x%04x)] corner:%d\n",
		...
		gauge->nafg_corner);
#endif 
	bm_debug("[fg_bat_nafg][%s] mV:Reg[%d:%d] [b10:%d][26_16(0x%04x) 15_00(0x%04x)]\n",__func__, nag_c_dltv_mv_value, nag_c_dltv_reg_value,bcheckbit10, nag_c_dltv_value_h, nag_c_dltv_value);
...
int get_battery_id(struct platform_device *pdev)
{
	struct iio_channel *bat_id_vol;
	int rc, i = 0, bat_id_mv = 0;
	int battery_id = 0;

	bat_id_vol = iio_channel_get(&pdev->dev, "batteryID-channel");
	if (IS_ERR_OR_NULL(bat_id_vol)) {
		pr_err("iio_channel_get batt_id fail\n");
		return -ENODEV;
	}
	pr_info("bat_id_vol: %d\n",bat_id_vol);
	rc = iio_read_channel_processed(bat_id_vol, &bat_id_mv);
	if (rc < 0) {
		pr_err("iio read batt_id fail\n");
		return rc;
	}
	for(;i < ARRAY_SIZE(id_range); ++i) {
		if((bat_id_mv > id_range[i].v_min) && (bat_id_mv < id_range[i].v_max)) {
			battery_id = i;
			break;
		}
	}
	pr_info("bat_id_uv: %d,battery_id: %d\n", bat_id_mv, battery_id);
	return battery_id;
}
static int mt6377_gauge_probe(struct platform_device *pdev)
{
	...
	if (battery_psy_init(pdev))
		return -ENOMEM;
	gauge->gm->battery_id = get_battery_id(pdev);
