...
#include "nopmi_mtk_battery_name.h"
...
int afc_low = 40;
int afc_ui = 160;
EXPORT_SYMBOL_GPL(afc_high);
EXPORT_SYMBOL_GPL(afc_low);
EXPORT_SYMBOL_GPL(afc_ui);
...
static ssize_t afc_high_show(struct device *dev, struct device_attribute *attr,
                           char *buf)
{
...
static ssize_t afc_high_store(struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t size)
{
	int temp = 0;
...
static DEVICE_ATTR_RW(afc_high);

static ssize_t afc_low_show(struct device *dev, struct device_attribute *attr,
                           char *buf)
{
...
static ssize_t afc_low_store(struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t size)
{
    int temp = 0;
...
static DEVICE_ATTR_RW(afc_low);  


static ssize_t afc_ui_show(struct device *dev, struct device_attribute *attr,
                           char *buf)
{
...
static ssize_t afc_ui_store(struct device *dev, struct device_attribute *attr,
                        const char *buf, size_t size)
{
    int temp = 0;
...
static bool charger_init_algo(struct mtk_charger *info)
{
	...
	info->chg1_dev = get_charger_by_name("charger");
	if (info->chg1_dev)
	...
	if (info->config == DUAL_CHARGERS_IN_SERIES) {
		...
		info->dvchg1_dev = get_charger_by_name("mtk-primary_dvchg");
		if (info->dvchg1_dev)
	...
	if (boot_mode == 8 || boot_mode == 9) {
		...
		if (vbus >= 0 && vbus < 2500 && !mtk_is_charger_on(info) && !info->pd_reset) {
			...
			while (1) {
				...
				if (info->is_suspend == false && counter >=150 && get_vbus(info) < 2500) {
					chr_err("%s, not in suspend, shutdown,hardreset:%d\n", __func__, info->pd_reset);
					kernel_power_off();
					break;
				} else if (get_vbus(info) > 2500) {
					break;
				} else{
...
static void charger_status_check(struct mtk_charger *info)
{
	...
	chr_err("%s t_c_c online.intval=%d status.intval=%d charging =%d info->is_charging=%d\n", __func__,online.intval,status.intval,charging,info->is_charging);
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		chr_err("charger_routine_thread t_c_c is_module_init_done =%d \n",
			    is_module_init_done);
		...
		chr_err("charger_routine_thread t_c_c disable charging %d %d %d\n",
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	if (ret)
		goto _out;
	ret = device_create_file(&(pdev->dev), &dev_attr_afc_high);
	ret = device_create_file(&(pdev->dev), &dev_attr_afc_low);
	ret = device_create_file(&(pdev->dev), &dev_attr_afc_ui);
	...
	int idx, bool en)
{
	...
	switch (idx) {
	case CHG1_SETTING:
		chg_dev = get_charger_by_name("mtk-charger");
		break;
	case CHG2_SETTING:
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	int i;
	char *name = NULL;
	struct adapter_device  *tmp_adapter = NULL;

	chr_err("%s: starts\n", __func__);

  	tmp_adapter = get_adapter_by_name("pd_adapter");
	if (!tmp_adapter) {
		chr_err("%s: No pd adapter found,try\n", __func__);
		// return -EPROBE_DEFER;
	}
