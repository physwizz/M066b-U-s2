...
static struct mutex afc_mutex_lock;
#else
...
int afc_set_voltage_workq(unsigned int afc_code)
{
	...
	gpio_direction_output(afc_switch, 1);
	...
	do {
		...
		for (i = 0; i < AFC_COMM_CNT; i++) {
			if(i>0) {
				if(gpio_get_value(afc_switch) == 0) {
					pr_info("afc canceled\n");
					goto afc_end;
				}
				msleep(38);
			}
...
afc_end:
	gpio_direction_input(afc_data);
	gpio_direction_output(afc_switch, 0);

	return ret;
}
EXPORT_SYMBOL_GPL(afc_set_voltage_workq);

int afc_cancel_voltage_workq(void)
{
	if (afc_driver_probe)
		return afc_driver_probe;
...
EXPORT_SYMBOL_GPL(afc_cancel_voltage_workq);
