...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	bs_data = &gm->bs_data;
	bs_data->psd.name = MTK_BATTERY_PSY_NAME,
	bs_data->psd.type = POWER_SUPPLY_TYPE_UNKNOWN; //POWER_SUPPLY_TYPE_BATTERY;
	...
	struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (strcmp(psy->desc->name, MTK_BATTERY_PSY_NAME) == 0) {
		if (gm != NULL) {
			sdc = container_of(
...
int battery_init(struct platform_device *pdev)
{
	...
	gm->battery_sysfs_field_tbl = battery_sysfs_field_tbl;
