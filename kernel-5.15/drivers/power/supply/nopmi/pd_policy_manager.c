...
#define BATT_MAX_CHG_VOLT0		4440
#define BATT_MAX_CHG_VOLT1		4410
#define BATT_MAX_CHG_VOLT2		4390
#define BATT_MAX_CHG_VOLT3		4370
#define BATT_MAX_CHG_VOLT4		4320
#define BATT_FAST_CHG_CURR		5500
...
#define PM_WORK_RUN_FUNC_INTERVAL	700
...
static const struct pdpm_config pm_config = {
	...
	//The starting voltage is 3500mV to prevent the floating voltage from decreasing 100mV.
	.min_vbat_for_cp	= 3500 - 100,

	.cp_sec_enable		= false,
	.fc2_disable_sw		= true,
};
//extern void upm6922_enter_hiz(void);
//extern void upm6922_exit_hiz(void);
...
static const char * const nopmi_iio_chan_name[] = {
	...
	[NOPMI_CHARGING_ENABLE] = "battery_charging_enabled"
};
...
static const char * const cp_iio_chan_name[] = {
	...
	[CHARGE_PUMP_SP2130_BATTERY_PRESENT] = "upm6722_battery_present",
	[CHARGE_PUMP_SP2130_VBUS_PRESENT] = "upm6722_vbus_present",
	[CHARGE_PUMP_SP2130_BATTERY_VOLTAGE] = "upm6722_battery_voltage",
	[CHARGE_PUMP_SP2130_BATTERY_CURRENT] = "upm6722_battery_current",
	[CHARGE_PUMP_SP2130_BATTERY_TEMPERATURE] = "upm6722_battery_temperature",
	[CHARGE_PUMP_SP2130_BUS_VOLTAGE] = "upm6722_bus_voltage",
	[CHARGE_PUMP_SP2130_BUS_CURRENT] = "upm6722_bus_current",
	[CHARGE_PUMP_SP2130_BUS_TEMPERATURE] = "upm6722_bus_temperature",
	[CHARGE_PUMP_SP2130_DIE_TEMPERATURE] = "upm6722_die_temperature",
	[CHARGE_PUMP_SP2130_ALARM_STATUS] = "upm6722_alarm_status",
	[CHARGE_PUMP_SP2130_FAULT_STATUS] = "upm6722_fault_status",
	[CHARGE_PUMP_SP2130_VBUS_ERROR_STATUS] = "upm6722_vbus_error_status",
	//[CHARGE_PUMP_SP2130_CHIP_VENDOR] = "upm6722_chip_vendor",
...
static const char * const cp_sec_iio_chan_name[] = {
	...
	[CHARGE_PUMP_SP2130_BATTERY_PRESENT] = "upm6722_battery_present_slave",
	[CHARGE_PUMP_SP2130_VBUS_PRESENT] = "upm6722_vbus_present_slave",
	[CHARGE_PUMP_SP2130_BATTERY_VOLTAGE] = "upm6722_battery_voltage_slave",
	[CHARGE_PUMP_SP2130_BATTERY_CURRENT] = "upm6722_battery_current_slave",
	[CHARGE_PUMP_SP2130_BATTERY_TEMPERATURE] = "upm6722_battery_temperature_slave",
	[CHARGE_PUMP_SP2130_BUS_VOLTAGE] = "upm6722_bus_voltage_slave",
	[CHARGE_PUMP_SP2130_BUS_CURRENT] = "upm6722_bus_current_slave",
	[CHARGE_PUMP_SP2130_BUS_TEMPERATURE] = "upm6722_bus_temperature_slave",
	[CHARGE_PUMP_SP2130_DIE_TEMPERATURE] = "upm6722_die_temperature_slave",
	[CHARGE_PUMP_SP2130_ALARM_STATUS] = "upm6722_alarm_status_slave",
	[CHARGE_PUMP_SP2130_FAULT_STATUS] = "upm6722_fault_status_slave",
	[CHARGE_PUMP_SP2130_VBUS_ERROR_STATUS] = "upm6722_vbus_error_status_slave",
	//[CHARGE_PUMP_SP2130_CHIP_VENDOR] = "upm6722_chip_vendor_slave",
...
static int pd_policy_set_nopmi_usb(bool online_val)
{
	int ret = 0;
	...
	if (propval.intval) {
		ret = power_supply_set_property(usb_psy, POWER_SUPPLY_PROP_PRESENT, &propval);
		if (ret < 0)
			pr_err("set usb present fail ret:%d\n", ret);
		ret = power_supply_set_property(usb_psy, POWER_SUPPLY_PROP_ONLINE, &propval);
		if (ret < 0)
...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
	...
	if (apdo_idx != -1){
		...
		ret = usbpd_pps_enable_charging(pdpm, true,5000, 2000);
		if (ret != TCP_DPM_RET_SUCCESS)
			return false;
		return true;
	}
...
static int usbpd_pm_enable_sw(struct usbpd_pm *pdpm, bool en)
{
	...
	ret = usbpd_set_iio_channel(pdpm, NOPMI,
			NOPMI_CHARGING_ENABLE, val.intval);
...
static int usbpd_pm_enable_cp(struct usbpd_pm *pdpm, bool enable)
{
	...
	if (enable) {
		...
		charger_dev_enable_hiz(pdpm->sw_chg, true);
		//usbpd_set_iio_channel(pdpm, MAIN, CP_MAIN_CHARGE_ENABLED, 0);
		pr_err("switch charger disable\n");
...
int usbpd_update_ibat_curr(struct usbpd_pm *pdpm)
{
	...
	if (PD_PM_STATE_FC2_TUNE == pdpm->state) {
		...
		if (!ret) {
			pdpm->cp.vbat_volt_sw = (int)(val.intval/1000);
			pdpm->cp.vbat_volt = pdpm->cp.vbat_volt_sw;
		}
	}

	if (PD_PM_STATE_FC2_TUNE == pdpm->state)
		pr_err("ibat_curr_fg:%d vbat_volt_fg:%d\n", pdpm->cp.ibat_curr_sw, pdpm->cp.vbat_volt_sw);
	else
		pr_err("ibat_curr_fg:%d\n", pdpm->cp.ibat_curr_sw);
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	*/
	...
	if (pdpm->cp.vbat_volt > CP_constant_voltage)
		...
		step_vbat = pm_config.fc2_steps;;
	pr_err("vbat:%d lmt:%d step:%d", pdpm->cp.vbat_volt, CP_constant_voltage, step_vbat);

	/* battery charge current loop*/
	if (pdpm->cp.ibat_curr < fcc_limit - 20)
		step_ibat = pm_config.fc2_steps;
	else if (pdpm->cp.ibat_curr > fcc_limit + 80)
		step_ibat = -pm_config.fc2_steps;
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	pr_err("pm_sm vbus_vol:%d ibus_curr_cp:%d vbat_vol:%d ibat_curr:%d\n",
		pdpm->cp.vbus_volt, pdpm->cp.ibus_curr_cp, pdpm->cp.vbat_volt, pdpm->cp.ibat_curr);
	switch (pdpm->state) {
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->cp.vbat_volt < pm_config.min_vbat_for_cp) {
			...
			if (!pdpm->is_disable_quick_charge) {
				usbpd_select_pdo(pdpm,9000, 2750); // 9V-1A
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (pdpm->cp.vbus_error_low || pdpm->cp.vbus_volt < pdpm->cp.vbat_volt*2 + BUS_VOLT_INIT_UP - 50) {
			tune_vbus_retry++;
			pdpm->request_voltage += max(20, ((pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP) - pdpm->cp.vbus_volt) / 20 * 20);
			usbpd_select_pdo(pdpm,pdpm->request_voltage, pdpm->request_current);
			...
			tune_vbus_retry++;
			pdpm->request_voltage -= max(20, (pdpm->cp.vbus_volt - (pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP)) / 20 * 20);
			usbpd_select_pdo(pdpm,pdpm->request_voltage,pdpm->request_current);
	...
	case PD_PM_STATE_FC2_EXIT:
		...
		usbpd_check_pca_chg_swchg(pdpm);
		charger_dev_enable_hiz(pdpm->sw_chg, false);
		// usbpd_set_iio_channel(pdpm, MAIN, CP_MAIN_CHARGE_ENABLED, 1);
		if (recover)
		...
		else
		{
			/*if(NOPMI_CHARGER_IC_MAXIM == nopmi_get_charger_ic_type()){
				...
				usbpd_pps_enable_charging(pdpm,true,5500,2000);
			}
			rc = 1;
#ifdef CP_RETRY_RESTORE
			pdpm->cp_retry_restore_count = 0;
...
//	usbpd_update_ibus_curr(pdpm);
	...
	if (!usbpd_pm_sm(pdpm) && pdpm->pd_active) {
		...
		} else if ((pre_state <= PD_PM_STATE_FC2_ENTRY_2) && (!pdpm->pdo_request_count)) {
			if ((pre_state != pdpm->state) || (pre_state == PD_PM_STATE_FC2_ENTRY_2)) {
				pr_info("no pdo request,can quickly next,%d,%d,%d", pre_state, pdpm->state, pdpm->pdo_request_count);
				schedule_delayed_work(&pdpm->pm_work, msecs_to_jiffies(1));
			} else {
				schedule_delayed_work(&pdpm->pm_work, msecs_to_jiffies(PM_WORK_RUN_INTERVAL));
			}
		} else {
...
static void usb_psy_change_work(struct work_struct *work)
{
	...
	if (!pdpm->pd_active && pdpm->is_pps_en_unlock == true){
		...
		pd_policy_set_nopmi_usb(false);
	}

}

static int usbpd_psy_notifier_cb_handle(struct usbpd_pm *pdpm)
{
...
static void usbpd_psy_notifier_work(struct work_struct *work)
{
	struct usbpd_pm *pdpm = container_of(work, struct usbpd_pm,
		usbpd_psy_notifier_work);

	usbpd_psy_notifier_cb_handle(pdpm);
}
			...
			unsigned long event, void *data)
{
	...
	schedule_work(&pdpm->usbpd_psy_notifier_work);
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	bbc_psy = power_supply_get_by_name("charger");
	...
	INIT_WORK(&pdpm->usbpd_psy_notifier_work, usbpd_psy_notifier_work);
