...
/* add sdcard slot info for factory mode at 2024/09/23
 *    begin
 *    */
static struct msdc_host *static_cardhost = NULL;
static ssize_t card_slot_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mmc_host *mmc = mmc_from_priv(static_cardhost); // checked new style:  don't use cardhost->mmc;


	pr_err("card_slot_status_show enter! mmc_host name:%s\n %s\n ", mmc_hostname(mmc),__FILE__); //mmc_hostname is in /include/mmc/host.h
	return snprintf(buf, PAGE_SIZE, "%d\n", mmc_gpio_get_cd(mmc));  //mmc_gpio_get_cd is in /include/mmc/slot-gpio.h
}
static DEVICE_ATTR(card_slot_status, S_IRUGO , card_slot_status_show, NULL);
static int card_slot_init_device_name(void)    //(struct platform_device *pdev)
{
	int32_t error = 0;
	// struct mmc_host *mmc;
	// struct msdc_host *host;
	...
	error = device_create_file(&mmc->class_dev, &dev_attr_card_slot_status); // checked class_dev in host.h  . struct mmc_host{   . and  marco DEVICE_ATTR(card_slot_status...)
	if (error < 0) {
...
static void card_slot_exit_device_name(void)    //(struct platform_device *pdev)
{

	struct mmc_host *mmc = mmc_from_priv(static_cardhost);  //struct mmc_host *mmc = static_cardhost->mmc; //wrong
	// pdev! pointer
	// struct mmc_host *mmc;
	// struct msdc_host *host;
	...
	device_remove_file(&mmc->class_dev, &dev_attr_card_slot_status);  //checked same as create file.
	static_cardhost = NULL;
}
...
/* add sdcard slot info for factory mode start 2024-9-23  */
	static_cardhost = mmc_priv(mmc);

	ret = card_slot_init_device_name();   //mine
	if (ret < 0) {
...
/* add sdcard slot info for factory mode start 2024-9-23  */
	card_slot_exit_device_name();   // mine
/* add sdcard slot info for factory mode end  */
