...
ssize_t SS_headset_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int type = 0;

	type = (char)accdet->cable_type;
	pr_info("[ZS] headset_state_A05Score_show earjack_state value %s\n" ,(char)accdet->cable_type);

	return sprintf(buf, "%d\n", type);
}

ssize_t SS_headset_state_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	return len;
}

static struct device_attribute classic_earjack_state_attrs[] = {
	__ATTR(state, 0644, SS_headset_state_show, SS_headset_state_store),
};
...
static void eint_work_callback(struct work_struct *work)
{
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		...
		accdet_init();
                #ifndef CONFIG_LCT_FACTORY_VERSION
		msleep(500);
                #endif
		enable_accdet(0);
